...
> = {
  ...
  'Near Threatened': {
    ...
    color:
      'bg-lime-50 text-lime-700 ring-lime-600/20 dark:bg-lime-500/10 dark:text-lime-400 dark:ring-lime-400/30',
    dot: 'bg-lime-500',
    level: 2,
  },
  Vulnerable: {
  ...
  Endangered: {
    label: 'Endangered',
    color:
      'bg-orange-50 text-orange-700 ring-orange-600/20 dark:bg-orange-500/10 dark:text-orange-400 dark:ring-orange-400/30',
    dot: 'bg-orange-500',
    level: 4,
  },
...
function InfoCard({ icon, label, value }: InfoCardProps) {
  return (
    <div className="group flex items-start gap-3 rounded-xl border border-gray-200 bg-white/60 p-4 transition-colors hover:border-gray-300 hover:bg-white dark:border-gray-800 dark:bg-gray-900/40 dark:hover:border-gray-700 dark:hover:bg-gray-900/70">
      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gray-100 text-gray-600 transition-colors group-hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:group-hover:bg-gray-700">
        {icon}
      </div>
      <div className="min-w-0">
        ...
        <p className="mt-0.5 text-sm leading-relaxed text-gray-900 dark:text-white">
          {value}
        </p>
      </div>
    </div>
  ...
  return (
    <div className="mx-auto w-full max-w-xl space-y-6">
      ...
      <header className="space-y-3">
        <span className="inline-flex items-center gap-1.5 rounded-full bg-indigo-50 px-3 py-1 text-xs font-medium text-indigo-700 ring-1 ring-inset ring-indigo-600/20 dark:bg-indigo-500/10 dark:text-indigo-400 dark:ring-indigo-400/30">
          <SpeciesIcon />
          Species Profile
        </span>
        <div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-gray-50 p-5 dark:border-gray-800 dark:bg-gray-900/50">
        ...
        <div className="mt-4">
          ...
          <div className="mt-1.5 flex justify-between text-[10px] font-medium uppercase tracking-wide text-gray-400 dark:text-gray-500">
            <span>Safe</span>
            <span>At Risk</span>
          </div>
        </div>
      ...
      <section className="space-y-3">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <InfoCard icon={<FamilyIcon />} label="Family" value={species.family} />
          <InfoCard icon={<OrderIcon />} label="Order" value={species.order} />
        </div>
        <InfoCard icon={<HabitatIcon />} label="Habitat" value={species.habitat} />
      </section>
    </div>
  )
}
